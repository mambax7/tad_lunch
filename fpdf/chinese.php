...
class pdf_chinese extends FPDF
{
    ...
    public function Row($data, $char_code, $fstyle, $fsize)
    {
        ...
        for ($i = 0; $i < count($data); $i++) {
            ...
            $fstyle[$i] = isset($fstyle[$i]) ? $fstyle[$i] : ' ';
            $fsize[$i] = isset($fsize[$i]) ? $fsize[$i] : '10';
            ...
            $this->Rect($x, $y, $w, $h);
            //Print the text
            $this->SetFont($char_code[$i], $fstyle[$i], $fsize[$i]);
            $this->MultiCell($w, $fsize[0] / 2, $data[$i], 0, $a);
    ...
    */
    ...
    public function AddBig5hwFont($family = 'Big5-hw')
    {
        for ($i = 32; $i <= 126; $i++) {
            $cw[chr($i)] = 500;
        }
