...
// 匯入使用者資料
  if ('upfile' == $act && $_FILES['userdata']['size'] > 0 && '' != $_FILES['userdata']['name']) {
      $fd = fopen($_FILES['userdata']['tmp_name'], 'rb');
      ...
      $i = 0; //存入筆數
        ...
        while ($tt = fgetcsv($fd, 2000, ',')) {
            if (0 == $i++) { //第一筆為抬頭
                if (_MA_LUNCH_ADD_ID != trim($tt[0]) || _MA_LUNCH_NV != trim($tt[1]) || _MA_LUNCH_ITEM != trim($tt[2]) || _MA_LUNCH_NUMBER != trim($tt[3]) || _MA_LUNCH_ADD_UNIT != trim($tt[4]) || _MA_LUNCH_ITEM != trim($tt[5]) || _MA_LUNCH_NUMBER != trim($tt[6]) || _MA_LUNCH_ADD_UNIT != trim($tt[7]) || _MA_LUNCH_ITEM != trim($tt[8]) || _MA_LUNCH_NUMBER != trim($tt[9]) || _MA_LUNCH_ADD_UNIT != trim($tt[10]) || _MA_LUNCH_ITEM != trim($tt[11]) || _MA_LUNCH_NUMBER != trim($tt[12]) || _MA_LUNCH_ADD_UNIT != trim($tt[13]) || _MA_LUNCH_ITEM != trim($tt[14]) || _MA_LUNCH_NUMBER != trim($tt[15]) || _MA_LUNCH_ADD_UNIT != trim($tt[16]) || _MA_LUNCH_ITEM != trim($tt[17]) || _MA_LUNCH_NUMBER != trim($tt[18]) || _MA_LUNCH_ADD_UNIT != trim($tt[19]) || _MA_LUNCH_ITEM != trim($tt[20]) || _MA_LUNCH_NUMBER != trim($tt[21]) || _MA_LUNCH_ADD_UNIT != trim($tt[22]) || _MA_LUNCH_ITEM != trim($tt[23]) || _MA_LUNCH_NUMBER != trim($tt[24]) || _MA_LUNCH_ADD_UNIT != trim($tt[25]) || _MA_LUNCH_ITEM != trim($tt[26]) || _MA_LUNCH_NUMBER != trim($tt[27]) || _MA_LUNCH_ADD_UNIT != trim($tt[28]) || _MA_LUNCH_ITEM != trim($tt[29]) || _MA_LUNCH_NUMBER != trim($tt[30]) || _MA_LUNCH_ADD_UNIT != trim($tt[31])) {
                    redirect_header($_SERVER[PHP_SELF], 10, _MA_LUNCH_IN_ERR);
                }
                continue;
            }
...
// 匯入樣版檔
  if ('updata_temp' == $act) {
      $fd = fopen("../sql/lunch_txt_{$xoopsConfig['language']}.csv", 'rb');
...
//修改
if (isset($_POST['op_sql']) && ('modify_one' == $_POST['op_sql'])) {
    $sql = '
    	replace into ' . $xoopsDB->prefix('tad_lunch_dish') . "
    	(id,kind,dish,item_name_1,item_number_1,item_unit_1,item_name_2,item_number_2,item_unit_2,item_name_3,item_number_3,item_unit_3,item_name_4,item_number_4,item_unit_4,item_name_5,item_number_5,item_unit_5,item_name_6,item_number_6,item_unit_6,item_name_7,item_number_7,item_unit_7,item_name_8,item_number_8,item_unit_8,item_name_9,item_number_9,item_unit_9,item_name_10,item_number_10,item_unit_10,change_date)
     	values
...
//新增
if (isset($_POST['op_sql']) && ('add' == $_POST['op_sql'])) {
    $sql = '
