...
class zipfile
{
    ...
    public function addFileAndRead($file)
    {
        if (is_file($file)) {
    ...
    public function change_str($source, $is_reference = 1)
    {
        ...
        if ($is_reference) {
            $ttt = '';
            for ($i = 0; $i < mb_strlen($temp_str); $i++) {
    ...
    public function change_temp($arr, $source, $is_reference = 1)
    {
        $temp_str = $source;
    ...
    //�S���ഫ UTF-8�A�Ҳղ��͵{���|�Ψ�C
    public function change_temp2($arr, $source)
    {
        $temp_str = $source;
        while (list($id, $val) = each($arr)) {
            //$val =iconv("Big5","UTF-8",$val);
    ...
    //XML ����ѷ��ഫ
    public function xml_reference_change($text)
    {
        $sw = ['&' => '&amp;', '<' => '&lt;', '>' => '&gt;', '"' => '&quot;', "'" => '&apos;'];
    ...
    //�v�r�ഫutf8�r�ꬰbig5
    public function utf8_2_big5($utf8_str)
    {
        $i = 0;
        $len = mb_strlen($utf8_str);
        $big5_str = '';
        for ($i = 0; $i < $len; $i++) {
            $sbit = ord(mb_substr($utf8_str, $i, 1));
            //echo $sbit."<br>";
            if ($sbit < 128) {
                $big5_str .= mb_substr($utf8_str, $i, 1);
            } elseif ($sbit > 191 && $sbit < 224) {
                $new_word = iconv('UTF-8', 'Big5', mb_substr($utf8_str, $i, 2));
                $big5_str .= ('' == $new_word) ? '��' : $new_word;
                //echo $big5_str."<br>";
                $i++;
            } elseif ($sbit > 223 && $sbit < 240) {
                $new_word = iconv('UTF-8', 'Big5', mb_substr($utf8_str, $i, 3));
                $big5_str .= ('' == $new_word) ? '��' : $new_word;
                $i += 2;
            } elseif ($sbit > 239 && $sbit < 248) {
                $new_word = iconv('UTF-8', 'Big5', mb_substr($utf8_str, $i, 4));
                $big5_str .= ('' == $new_word) ? '��' : $new_word;
                $i += 3;
            }
        }
